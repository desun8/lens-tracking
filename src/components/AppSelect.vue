...
<template>
  <div class="relative grid gap-2 group">
    <label
      v-if="$slots.default"
      :for="id"
      class="text-sm font-medium text-slate-900 dark:text-slate-100 transition-colors duration-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400"
    >
      <slot></slot>
    </label>
    ...
    <div class="relative group">
      ...
      <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
        <svg
          class="w-4 h-4 text-slate-500 dark:text-slate-400 transition-colors duration-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400"
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
          stroke-width="2"
